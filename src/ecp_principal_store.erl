...
handle_call({new, Principal}, _From, State) ->
  Fun = fun() ->
    Updated = case Principal#ecp_principal.date_created == undefined of 
		true -> Principal#ecp_principal{date_created=ecp_time:current_timestamp()} ;
		_ -> Principal
	end,
...
handle_call({lookup_string, Id}, _From, State) ->
  case no_gen_server_lookup(Id) of
    {found, P} ->
      ...
      {_Counter, Result} = lists:foldl(fun(Field, {Counter,Accum}) -> {Counter + 1, io_lib:format("~s~n    ~s: ~p",[Accum, Field, element(Counter, P)])} end, {2,Start}, record_info(fields, ecp_principal)),
...
handle_call({deactivate, Principal}, _From, State) ->
  Fun = fun() ->
    Updated = Principal#ecp_principal{date_deactivated=ecp_time:current_timestamp()},
    mnesia:write(Updated),
    {cancelled, Updated}
  end,        
  Reply = mnesia:sync_dirty(Fun),
...
handle_call({activate, Principal}, _From, State) ->
  Fun = fun() ->
    Updated = Principal#ecp_principal{date_deactivated=undefined},
    mnesia:write(Updated),
    {cancelled, Updated}
  end,        
  Reply = mnesia:sync_dirty(Fun),
...
handle_call({delete, PrincipalId}, _From, State) ->
	Fun = fun() ->
		mnesia:delete({ecp_principal, PrincipalId}),
		{ deleted, PrincipalId }
	end,
	Reply = mnesia:sync_dirty(Fun),
	{ reply, Reply, State }. 
...
no_gen_server_lookup_all() ->
  mnesia:dirty_all_keys(ecp_principal).
  
no_gen_server_lookup(Id) ->
  case mnesia:dirty_read(ecp_principal, Id) of
    [Found] ->
      {found, Found};
    [] ->
      none
...
%
get_all() ->
	AllIds = mnesia:dirty_all_keys(ecp_principal),
	lists:map(
		fun (Key) ->
			[ Subscription ] = mnesia:dirty_read(ecp_principal, Key),
			Subscription
		end,
		AllIds
	).
