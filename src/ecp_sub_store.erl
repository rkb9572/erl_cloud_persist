...
init([]) ->
  mnesia:subscribe({table, ecp_sub, simple}),
  {ok, []}.
...
handle_call({new_sub, Subscription}, _From, State) ->
  Fun = fun() ->
	  Id = case Subscription#ecp_sub.id == undefined of
		  true -> 
			  {id,TempId} = pe_id_broker:get_next(),
			  TempId;
		  _ -> Subscription#ecp_sub.id
	  end,
    QueueName = "Sub-" ++ Id,
	  UpdatedSub = case Subscription#ecp_sub.date_created == undefined of
		  true -> Subscription#ecp_sub{id=Id,queue_name=QueueName, date_created=pe_time:current_timestamp()} ;
		  _ -> Subscription#ecp_sub{id=Id, queue_name=QueueName}
	  end,
...
handle_call({get_all_for_principal, PrincipalId}, _From, _State) ->
  Result = mnesia:dirty_match_object(ecp_sub, {ecp_sub, '_', PrincipalId, '_', '_', '_', '_', '_', '_', '_'}),
  {reply, Result, _State};
  
handle_call({get_all_active_for_principal, PrincipalId}, _From, _State) ->
  Result = mnesia:dirty_match_object(ecp_sub, {ecp_sub, '_', PrincipalId, '_', '_', '_', '_', '_', undefined, '_'}),
  {reply, Result, _State};

handle_call({find_by_duplication_key, Key}, _From, _State) ->
  Result = mnesia:dirty_match_object(ecp_sub, {ecp_sub, '_', '_', '_', '_', '_', '_', '_', '_', Key}),
  {reply, Result, _State};
...
handle_call({lookup_string, Id}, _From, State) ->
  case no_gen_server_lookup(Id) of
    {found, Sub} ->
      ...
      {_Counter, Result} = lists:foldl(fun(Field, {Counter,Accum}) -> {Counter + 1, io_lib:format("~s~n    ~s: ~p",[Accum, Field, element(Counter, Sub)])} end, {2,Start}, record_info(fields, ecp_sub)),
...
handle_call({cancel, Subscription}, _From, State) ->
  Fun = fun() ->
    UpdatedSub = Subscription#ecp_sub{date_cancelled=pe_time:current_timestamp()},
    %mnesia:write(UpdatedSub), %A logic delete, but keep the record.
    Id = ecp_sub:get(id, Subscription),
    mnesia:delete({ecp_sub, Id}),
...
handle_info(_Msg, State) ->
  error_logger:info_msg("ecp_sub_store:  Unknown handle_info: ~p~n", [_Msg]),
  {noreply, State}.
...
do_find_by_message_type (MessageType) -> 
	AllSubscriptions = mnesia:dirty_all_keys(ecp_sub),
...
check_subscription(Key, MessageType) ->
	[ Subscription ] = mnesia:dirty_read(ecp_sub, Key),
...
get_all_subscriptions() ->
	AllIds = mnesia:dirty_all_keys(ecp_sub),
	lists:map(
		fun (Key) ->
			[ Subscription ] = mnesia:dirty_read(ecp_sub, Key),
			Subscription
